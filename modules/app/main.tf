...
resource "null_resource" "ansible" {
  ...
  provisioner "remote-exec" {
    ...
    inline = [
      ...
      "ansible-pull  -i localhost, -U https://github.com/awsdevops-sbs/ansible.git  expense.yml -e role_name=${var.component}  -e env=${var.env} -e vault_token=${var.vault_token} -e  @~/secret.json -e @~/app.json -e @~/newrelic_key.json"
